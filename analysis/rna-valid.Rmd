...
params:
  title: Whole-genome RNA-Seq data validation
  author: Javon
  date: January 2, 2021
  readCountFile: null
  idCol: 1
...
test = FALSE
if (test) {
  inputParams$readCountFile <- 'F:/projects/tissue-regen/analysis/fat-mm/ana-081821/rna/wt/data/expr/count.gene.csv'
  inputParams$sampGrpSpecFile <- 'F:/projects/tissue-regen/analysis/fat-mm/ana-081821/rna/wt/data/samp-grp.csv'
  # inputParams$sampGrp1Meta <- '/scratch/ml-csm/projects/tissue-regen/analysis/mouse-cardiac-fibro/ana-072921/config/samp-grp1.meta.csv'
  inputParams$output <- 'F:/projects/tissue-regen/analysis/fat-mm/ana-081821/rna/wt/validation/results/gene'
  inputParams$logFile <- 'F:/projects/tissue-regen/analysis/fat-mm/ana-081821/rna/wt/validation/run-log/val-gene.log'
}
```
...
printSampGrpDetail <- function() {
  if (length(sampOfGrp) == 1) {
    ...
    str <- sprintf('%s and %s(N = %d)', str, grp, length(sog[[length(sog)]]))
    return(str)
  } else {
...
There are expression data of total `r printNum(nrow(inputData))` `r feature`s for `r printNum(ncol(inputData) - dataStartCol + 1)` samples in the input data file (i.e., "*readCountFile*"). The samples are: `r printNames(names(inputData)[dataStartCol:ncol(inputData)])`. There are `r printNum(length(sampUse))` samples found in the given "*sampGrpSpecFile*". Only these samples (, which may be a subset of those in the expression data file) will be included in the subsequent analysis and are: `r printNames(sampUse)`. There are/is `r printNum(length(sampOfGrp))` way`r if(length(sampOfGrp) > 1) {'s'}` of sample grouping found in the "*sampGrpSpecFile*". `r printSampGrpDetail()`. 
...
# remove features that had no expression at any sample
anyExpr <- apply(dataUse[, 2:ncol(dataUse)], 1, 
     function(x){if(length(which(x == 0)) == length(x)) return(FALSE) else return(TRUE)})
dataUse <- dataUse[anyExpr, , drop = FALSE] 
...
# filter based on tpm
keep <- apply(tpm, 1, 
            function(x){if(length(which(x >= minExpr)) < minNSamp) return(FALSE) else return(TRUE)})
dataUse <- dataUse[keep, , drop = FALSE]
cnts <- cnts[keep, ]
tpm <- tpm[keep, ]
...
The expression level in TPM of each `r feature` in all samples was calculated and saved to *data/tpm.csv*. There are `r printNum(length(which(!keep)))` `r feature`s with an expression level (in TPM) over `r minExpr` in less than `r printNum(minNSamp)` sample(s). These `r feature`s (if any) were excluded from the subsequent analysis. The total number of `r feature`s remained in the analysis is `r printNum(nrow(dataUse))`. 

```{r early-exit-3, results='asis'}
earlyExit(cnts)
```

These `r feature`s together with their read counts in all samples were saved to *data/data-used.csv*. The following is a box plot of read counts per `r feature` in all samples after filtering (*feat-rcnt-1.pdf*). 
...
The number of expressed `r feature`s was counted for each sample and saved to *data/num-of-efeat.csv*. A `r feature` with an expression level no less than `r minExpr` in a sample is considered as expressed in that sample. Below is a bar plot of the number of expressed `r feature`s in all samples (*num-efeat-bar-1.pdf*).   

```{r 'num-efeat-bar', fig.height=3, fig.width=7, dev=c('png', 'pdf')}
nEFeat <- apply(tpm, 2, function(x) length(which(x >= minExpr)))
write.csv(data.frame(Samp = names(nEFeat), Count = nEFeat),
          paste0(outDataDir, '/num-of-efeat.csv'),
          row.names = FALSE, quote = FALSE)

plotNEFeatBar <- function(nums, grpClr = NULL) {
  ...
  p <- ggplot(df, aes(x = samp, y = num, fill = grp)) + 
    ...
    labs(title = '', x = '', y = sprintf('Number of expressed %ss', feature))
  if (!is.null(grpClr)) {
    p <- p + scale_fill_manual(name = '', values = grpClr)
  }
...
plotNEFeatBar(nEFeat, sampGrpClr)
```

The following is the density plot of the number of expressed `r feature`s per sample (*num-efeat-dens-1.pdf*). The estimation of the density was done by grouping samples according to the first way of sample grouping found in the input "*sampGrpSpecFile*".

```{r 'num-efeat-dens', fig.height=5, fig.width=7,dev=c('png', 'pdf')}
plotNEFeatDens <- function(nums, grpClr = NULL) {
  grps <- factor(getSampGrp(names(nums)), levels = getGrps())
  df <- data.frame(num = nums, grp = grps)
  p <- ggplot(df, aes(x = num, colour = grp)) + 
    ...
    theme(legend.title = element_blank(), legend.text = element_text(size = 12),
          axis.text.x = element_text(size = 12), axis.text.y = element_blank(),
          axis.title.x = element_text(size = 12), axis.title.y = element_text(size = 12)) +
    labs(title = '', x = sprintf('Number of expressed %ss per sample', feature), 
...
plotNEFeatDens(nEFeat, sampGrpClr)
```
...
write.csv(pcaData, paste0(outDataDir, '/pca-embedding.csv'), 
          row.names = FALSE, quote = FALSE)

plotPca <- function(pcaData, sampLabel = FALSE, grpClr = NULL) {
...
plotPca(pcaData, grpClr = sampGrpClr)
```

```{r samp-pca-lbl, fig.height=5, fig.width=6, dev=c('png', 'pdf')}
plotPca(pcaData, sampLabel = TRUE, grpClr = sampGrpClr)
```
...
```{r 'tsne-nlbl', fig.height=5, fig.width=6, dev=c('png', 'pdf')}
perplexity <- 2
if (ncol(vstCnts) < 5) {
  perplexity <- 1
}
tSneObj <- Rtsne(t(vstCnts), initial_dims = 30, perplexity = perplexity, pca_scale = TRUE, theta = 0.0, max_iter = 2000,
                 Y_init = pcaData[, 1:2] / -20) # use scaled (by factor 20) first two PCs
tSneCoor <- tSneObj$Y
...
plotTSne <- function(coors, group1, group2 = NULL, sampLabel = FALSE, grpClr = NULL) {
  ...
  if (sampLabel) {
    p <- p + geom_label_repel(aes(label = rownames(tSneCoor)),
                      box.padding = 0.35,
                      point.padding = 1,
                      segment.color = 'grey50',
                      segment.alpha = 0.5,
...
if ('group2' %in% names(pcaData)) {
  plotTSne(tSneCoor, factor(pcaData$group1, levels = getGrps()), pcaData$group2, grpClr = sampGrpClr)
} else {
  plotTSne(tSneCoor, factor(pcaData$group1, levels = getGrps()), grpClr = sampGrpClr)
}
```

```{r 'tsne-lbl', fig.height=5, fig.width=6, dev=c('png', 'pdf')}
if ('group2' %in% names(pcaData)) {
  plotTSne(tSneCoor, factor(pcaData$group1, levels = getGrps()), pcaData$group2, sampLabel = TRUE, grpClr = sampGrpClr)
} else {
  plotTSne(tSneCoor, factor(pcaData$group1, levels = getGrps()), sampLabel = TRUE, grpClr = sampGrpClr)
}
```
